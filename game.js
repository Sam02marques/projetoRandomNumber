...
let secretNumber =  Math.floor(Math.random() * (100 - 1 + 1)) + 1; // FUNÇÃO NATIVA QUE GERA UM NUMERO INTEIRO ALEATÓRIO INCLUINDO O VALOR MÁXIMO E MÍNIMO
let attempt;
let numberAttempt = 1;
...
function identifier(){
    do{ // SERVE PARA EXECUTAR TODA ESSA LINHA DE CÓDIGO, O WHILE NÃO DEIXA ESSA EXECUÇÃO PARAR ENQUANTO A CONDIÇÃO NÃO É FEITA
        attempt = Number(prompt("Insira um número qualquer, de 1 a 100, e eu te direi se ele é maior ou menor do que o número que escolhi"));
        ...
        if(attempt === secretNumber){
            alert(`Parabéns!!!! VOCÊ ACERTOU O NÚMERO SECRETO, que é ${secretNumber}`);
        }
        ...
        if (attempt === secretNumber){ //OPERADOR TERNÁRIO QUE CRIA UMA CONDIÇÃO DE FRASE PARA QUANDO AS TENTATIVAS FOREM MAIORES QUE 1 E IGUAIS A 1
            let attemptPhrase = numberAttempt > 1? "tentativas para acertar" : "tentativa para acertar, IMPRESSIONANTE!";
            alert(`Você precisou de ${numberAttempt} ${attemptPhrase}`);
        }
